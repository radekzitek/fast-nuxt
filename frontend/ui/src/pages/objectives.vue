<template>
  <v-container>
    ...
    <v-dialog v-model="dialog" max-width="600px">
      <v-card>
        <v-card-title>
          <span class="text-h6">{{ dialogMode === 'edit' ? 'Edit Objective' : 'Create Objective' }}</span>
        </v-card-title>
        <v-card-text>
          <v-form ref="formRef" @submit.prevent="saveObjective">
            <v-text-field v-model="editedObjective.title" label="Title" required :rules="[rules.required]" />
            <v-textarea v-model="editedObjective.description" label="Description" />
            <v-row>
              <v-col>
                <v-select
                  v-model="editedObjective.level"
              ...
              <v-col>
                <v-select
                  v-model="editedObjective.confidentiality"
                  :disabled="!enumOptionsLoaded"
              ...
              <v-col>
                <v-select
                  v-model="editedObjective.strategic_perspective"
                  :disabled="!enumOptionsLoaded"
            ...
            <v-row>
              <v-col>
                <v-select
                  v-model="editedObjective.priority"
                  :disabled="!enumOptionsLoaded"
              ...
              <v-col>
                <v-select
                  v-model="editedObjective.status"
                  :disabled="!enumOptionsLoaded"
            ...
            <v-row>
              <v-col>
                <v-text-field
                  v-model="editedObjective.start_date"
                  label="Start Date"
                  required
                  :rules="[rules.required]"
                  type="date"
              ...
              <v-col>
                <v-text-field
                  v-model="editedObjective.target_completion_date"
                  label="Target Date"
                  required
                  :rules="[rules.required]"
                  type="date"
            ...
            <v-row>
              <v-col>
                <v-select
                  v-model="editedObjective.owner_id"
                  :disabled="!teamMembersLoaded"
                  item-title="title"
                  item-value="value"
                  :items="teamMembers.map(tm => ({ title: tm.name || (tm.first_name + ' ' + tm.last_name), value: tm.id }))"
                  label="Owner"
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="saveObjective">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script setup>
  ...
  async function saveObjective () {
    ...
    const rest = { ...editedObjective.value }
    const payload = {
      ...rest,
